...
# > sum(is.na(core.plm))
# [1] 16017
# > sum(is.na((core.plm$population)))  !
# [1] 60
# > sum(is.na(core.plm$fstgdp))   !
# [1] 200
# > sum(is.na(core.plm$gdp))
# [1] 534
# > sum(is.na(core.plm$gdp.gr))
# [1] 665 
# > sum(is.na(core.plm$consumption))
# [1] 1066
# > sum(is.na(core.plm$inflation))  !
# [1] 51
# > sum(is.na(core.plm$debtgdp))
# [1] 878
# > sum(is.na(core.plm$pbalance))
# [1] 1277
# > sum(is.na(core.plm$tariffs))
# [1] 985
# > sum(is.na(core.plm$kofglobal))
# [1] 881
# > sum(is.na(core.plm$koftrade))
# [1] 881
# > sum(is.na(core.plm$tradegdp))  !
# [1] 195
# > sum(is.na(core.plm$fdi))
# [1] 912
# > sum(is.na(core.plm$labor))
# [1] 348
# > sum(is.na(core.plm$gini))
# [1] 1014
# > sum(is.na(core.plm$unemployment))
# [1] 1889
# > sum(is.na(core.plm$institutions))
# [1] 419
# > sum(is.na(core.plm$judicial))  !
# [1] 301
# > sum(is.na(core.plm$media))  !
# [1] 301
# > sum(is.na(core.plm$electoral))  !
# [1] 301
# > sum(is.na(core.plm$electoral))  !
# [1] 301
# > sum(is.na(core.plm$currcrisis))
# [1] 1104
# > sum(is.na(core.plm$debtcrisis))
# [1] 1103
# > sum(is.na(core.plm$bankcrisis))
# [1] 200
# > sum(is.na(core.plm$conflicts))
# [1] 332
# > sum(is.na(core.plm$war))
# [1] 118
# > sum(is.na(core.plm$populist))
# [1] 0
# sum(is.na(core.plm$rightpop))
# [1] 0
# sum(is.na(core.plm$leftpop))
# [1] 0
# sum(is.na(core.plm$populist5))
# [1] 0
# sum(is.na(core.plm$populist15))
# [1] 0
# sum(is.na(core.plm$rightpop5))
# [1] 1
# sum(is.na(core.plm$rightpop15))
# [1] 1
# sum(is.na(core.plm$leftpop5))
# [1] 0
# sum(is.na(core.plm$leftpop15))
# [1] 0


# We have duplicate Country x Year Combinations: Warning message:
# In pdata.frame(data, index = c("Country", "year")) :
#   duplicate couples (id-time) in resulting pdata.frame
# to find out which, use, e.g., table(index(your_pdataframe), useNA = "ifany")
# 
# To solve the problem we go this way:
# duplicate_combinations <- table(index(data), useNA = "ifany")
# duplicates <- as.data.frame(duplicate_combinations)
# summary(duplicates)
# duplciates2 <- dplyr::filter(duplicates, duplicates$Freq == 2) # because 2 is the largest freq (from summary)
# 51 cduplicate combinations for Romania: We delete all values that have NA in col Count
# Problem is solved above before mergeing the data: The Problem is the kof data frame, containing two variables
# Other ideas were:
# duplicate_row_names <- rownames(data)[duplicated(substring(rownames(data), nchar(rownames(data))-1))]
# pdata <- data[!(rownames(data) %in% duplicate_row_names), ]
# test <- data[complete.cases(data$Count), ]
# test <- pdata <- pdata[!(data$Country == Romania & pdata$col2 == value2), ]
# test <-subset(data, Count!= "NA" & Country!="Romania")
# Problem in the end: Kof data had code ROM and other data had code ROU



# Handling missing values with 

# https://stats.stackexchange.com/questions/473176/multiple-imputation-multi-level-time-series-panel-data

# mice, pan, amelia II, 

https://cran.r-project.org/web/packages/Amelia/
  
https://stats.stackexchange.com/questions/101178/handling-missing-data-in-a-time-series

https://stackoverflow.com/questions/72545436/using-mice-for-panel-data-models
...
https://mixomics-users.discourse.group/t/what-does-the-error-system-is-computationally-singular-mean-in-plsda/323

https://datascienceplus.com/handling-missing-data-with-mice-package-a-simple-approach/
  
https://stats.stackexchange.com/questions/184486/treating-missing-values-in-panel-data-set

https://cran.r-project.org/web/packages/imputeTS/imputeTS.pdf 

# Easy imputation method
pre.process <- preProcess(impute, method = "knnImpute")
imputed <- predict(pre.process, impute)
sum(is.na(imputed))

arg <- dplyr::filter(impute, Country == "Argentina")

arg.omit <- na.omit(arg)

mean(arg.omit$consumption)

# representative rightpop / leftpop country?

# daten vom SCM zum plotten nehmen und neu scalen sprich, je nach treatment beginn dann t+1 etc.
# daten in einen dataframe packen und erneut plotten lassen




core.scm <- dplyr::select(data, Country, year, gdp, population, unemployment, gini, fdi, populist)# Easy imputation method
pre.process <- preProcess(impute, method = "knnImpute")
imputed <- predict(pre.process, impute)
sum(is.na(imputed))
...
core.scm <- dplyr::select(data, Country, year, gdp, population, unemployment, gini, fdi, populist)


# APPROACH: Impute NAs with method bagImpute (mean)
# CHECK WICH IMPUTATION MATHOD IS THE BEST!!!

pre.process <- preProcess(data_mscmt, method = "bagImpute")
data_mscmt_imputed <- predict(pre.process, data_mscmt)
sum(is.na(data_mscmt_imputed))
